...
bool VBOMesh::Initialize(const FbxMesh *pMesh)
{
	...
	char StrLog[1024];
	sprintf(StrLog, "lPolygonCount : %d\n", lPolygonCount);
	...
	sprintf(StrLog, "lPolygonVertexCount : %d\n", lPolygonVertexCount);
	OutputDebugStringA(StrLog);
    ...
    if (mAllByControlPoint)
    {
        ...
        for (int lIndex = 0; lIndex < lPolygonVertexCount; ++lIndex)
        {
			...
			sprintf(StrLog, "x : %d, y : %d, z : %d\n", lVertices[lIndex * VERTEX_STRIDE], lVertices[lIndex * VERTEX_STRIDE + 1], lVertices[lIndex * VERTEX_STRIDE +2]);
			OutputDebugStringA(StrLog);
